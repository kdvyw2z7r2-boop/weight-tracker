...
function PhotoViewerModal({
  ...
  const handleRecropExisting = async () => {
    ...
    setIsProcessingPhoto(true)
    try {
      await openCropFromUrl(photoUrl)
    } catch (loadError) {
      setError(loadError.message || 'Impossible de charger la photo.')
    } finally {
      setIsProcessingPhoto(false)
    }
  }
                  ...
                  <>
                    ...
                    {photoUrl ? (
                      <button
                        ...
                        disabled={isBusy}
                        className="press-button mt-2 h-11 w-full rounded-[14px] bg-bg-elevated text-[14px] font-medium text-text-primary disabled:opacity-40"
                      >
                        Recadrer
                      </button>
                    ) : null}
